...
t = once(t,
    ...
    'finance effective status')
t = once(t,
    '  if (status === "cancelled") return "Cancelado";',
    '  if (status === "cancelled") return "Cancelado";\n  if (status === "negotiated") return "Renegociada";',
    'finance label negotiated')
t = once(t,
    '{(["all","pending","overdue","paid","cancelled"] as Filter[]).map',
    '{(["all","pending","overdue","negotiated","paid","cancelled"] as Filter[]).map',
    'finance tabs')
t = once(t,
    '    {modal?.kind === "pay" &&',
...
# App.tsx — usa o estado atual já endurecido pelo núcleo Ultimate.
p = Path('src/App.tsx')
t = p.read_text(encoding='utf-8')
t = once(t,
...
t = once(t,
    ...
    'open invoices')
old_student_total = 'invoices.filter((item) => effectiveStatus(item) !== "paid").reduce'
if old_student_total in t:
    t = t.replace(old_student_total,
        'invoices.filter((item) => { const status = effectiveStatus(item); return status === "pending" || status === "overdue"; }).reduce')
p.write_text(t, encoding='utf-8')
