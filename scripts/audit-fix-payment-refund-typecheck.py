...
p = Path('supabase/functions/payment-refund/index.ts')
text = p.read_text(encoding='utf-8')
old = '''    if (leaseToken) await admin.rpc("service_fail_payment_refund_attempt", { target_payment: payment.id, target_lease_token: leaseToken, target_error: message }).catch(() => undefined);'''
new = '''    if (leaseToken) {
      try {
...
print('Correção do tratamento de falha do estorno aplicada.')
