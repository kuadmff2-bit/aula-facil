...
fn is_exit_backup(path: &PathBuf) -> bool {
    path.file_name()
        ...
        .is_some_and(|name| name.starts_with(EXIT_BACKUP_PREFIX) && name.ends_with(EXIT_BACKUP_SUFFIX))
}
...
pub fn exit_backup_paths(app: &tauri::AppHandle) -> Result<Vec<PathBuf>, String> {
    ...
    let entries = match fs::read_dir(&directory) {
        ...
        Err(error) => return Err(format!("Não foi possível listar os backups automáticos: {error}")),
    };
...
pub fn create_exit_backup(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    let timestamp = SystemTime::now()
        ...
        .as_millis();
    let destination = directory.join(format!("{EXIT_BACKUP_PREFIX}{timestamp:013}{EXIT_BACKUP_SUFFIX}"));
...
pub fn clear_exit_backups(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    match fs::remove_dir_all(directory) {
        ...
        Err(error) => Err(format!("Não foi possível remover os backups automáticos: {error}")),
    }
}
