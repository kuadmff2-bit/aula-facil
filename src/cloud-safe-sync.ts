...
async function pushSnapshot(schoolId: string, source: SchoolDatabase, role: CloudSyncRole) {
  ...
  if (canWriteAcademicCore(role)) {
    await upsertRows("classes", database.classes.map((item) => ({ id: item.id, school_id: schoolId, name: item.name, group_name: item.groupName ?? "", teacher: item.teacher, schedule: item.schedule, meeting_days: item.meetingDays ?? [], start_time: item.startTime || null, end_time: item.endTime || null, room: item.room, monthly_fee: item.monthlyFee, duration_type: item.durationType ?? "open_ended", duration_months: item.durationType === "fixed" ? item.durationMonths ?? null : null, end_date: item.durationType === "fixed" ? item.endDate ?? null : null, workload_hours: item.workloadHours ?? null, color: item.color, active: true, created_at: item.createdAt, deleted_at: null })));
